...
// Готовим блок с офферами
function createOffersTemplate(pointOffers, typeOffers) {
  const notCheckedOffers = typeOffers.filter((offer) => !pointOffers.some((pointOffer) => pointOffer.id === offer.id));
  let offersTemplate = '';
...
function createEditPointFormTemplate(point, allOffers) {
  ...
  for (const offerGroup of allOffers) {
    if (offerGroup.type === type) {
      typeOffers.push(...offerGroup.offers);
      break;
    }
  }

  // Получаем шаблон с офферами
  const offersTemplate = createOffersTemplate(pointOffers, typeOffers);
  const imageTemplate = createImageTemplate(point.destination.pictures);
...
export default class EditPointView {
  ...
  getTemplate() {
    return createEditPointFormTemplate(this.point, this.allOffers);
  }
